...
void AddItem::on_btnAdd_clicked()
{
    ...
    query.prepare("insert into groceryitems(Name, Quantity, Status, DOP, DOS, SellingPrice, PurchasePrice) values (:Name, :Quantity, :Status, :DOP, :DOS, :SellingPrice, :PurchasePrice)");
    query.bindValue(":Name", sName);
    query.bindValue(":Quantity", sQuantity);
    query.bindValue(":Status", sStatus);
    query.bindValue(":DOP", sDOP);
    query.bindValue(":DOS", sDOS);
    query.bindValue(":SellingPrice", sSellingPrice);
    query.bindValue(":PurchasePrice", sPurchasePrice);
